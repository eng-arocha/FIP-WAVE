...
-- agregações calculadas pela aplicação.
--
-- Idempotente. O backfill a partir das tabelas legacy (nível grupo) só
-- roda se essas tabelas existirem.
...
-- 2. Backfill condicional (só roda se as tabelas legacy existirem)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'planejamento_fisico') THEN
    EXECUTE $SQL$
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'planejamento_fat_direto') THEN
    EXECUTE $SQL$
      WITH det_com_peso AS (
        SELECT d.id AS det_id,
               t.grupo_macro_id AS grupo_id,
