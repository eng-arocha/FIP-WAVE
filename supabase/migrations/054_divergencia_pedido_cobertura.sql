...
COMMENT ON COLUMN solicitacoes_fat_direto.origem_divergencia_id IS
  'Quando preenchido, indica que esta solicitação foi criada automaticamente ' ||
  'pra cobrir a divergência de valor de uma NF lançada no pedido aqui referenciado.';
...
COMMENT ON COLUMN notas_fiscais_fat_direto.tipo_rejeicao IS
  'Razão da rejeição: divergencia_sem_saldo, fornecedor_errado, fora_de_escopo, outro. ' ||
  'Texto livre, mas usa-se valores padronizados pra filtro/relatório.';
