...
COMMENT ON COLUMN contratos.tolerancia_nf_valor IS
  'Tolerância (R$) para divergência entre valor da NF e saldo do pedido fat-direto. ' ||
  'NFs com diferença <= esta tolerância são aceitas com flag de divergência. ' ||
  'NFs com diferença > esta tolerância exigem override explícito.';
...
COMMENT ON COLUMN notas_fiscais_fat_direto.divergencia_valor IS
  'true se a NF teve divergência de valor (>R$0,01) em relação ao saldo do pedido, ' ||
  'mesmo dentro da tolerância configurada do contrato.';
COMMENT ON COLUMN notas_fiscais_fat_direto.divergencia_excedente IS
  'Valor (R$) que excedeu o saldo. Positivo = NF maior que saldo. ' ||
  'Pode ser negativo se a NF for menor que o saldo (e a divergência foi sinalizada).';
